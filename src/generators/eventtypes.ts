...
function parseDemoFile(path: string) {
  fs.readFile(path, function (err, buffer) {
    ...
    function eventNameToInterfaceName(name: string) {
      let camelCased = name.replace(/(^\w|_\w)/g, g => g.toUpperCase().replace('_', ''));
    ...
    demoFile.on('svc_GameEventList', e => {
      ...
      for (let desc of e.descriptors) {
        ...
        for (let key of desc.keys) {
          ...
          if (key.type == 1)
            ...
            throw `Unexpected event type ${key.type}`;
      ...
      for (let desc of e.descriptors) {
        let interfaceName = `INonSpecificGame${eventNameToInterfaceName(desc.name).substring(1)}`;
