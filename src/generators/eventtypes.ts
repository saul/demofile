...
type EventVarMapping = (
  eventName: string
) => Record<string, [string, string]> | null;

const mappings: Record<string, EventVarMapping | undefined> = {
...
function parseDemoFile(path: string) {
  fs.readFile(path, (err, buffer) => {
    ...
    demoFile.on("svc_GameEventList", e => {
      console.log(
        `/* eslint-disable @typescript-eslint/no-unsafe-member-access, @typescript-eslint/explicit-module-boundary-types, sonarjs/no-duplicated-branches */`
      );
      ...
      const perEventAnnotations: Record<string, string[]> = {};
      ...
      for (const desc of descriptors) {
        ...
        for (const key of desc.keys) {
          ...
          if (mapping != null) {
            ...
            for (const [extraVar, [extraTypeStr, funcName]] of Object.entries(
              extra
            )) {
      ...
      console.log(
        `export function annotateEvent(entities: Entities, eventName: string, event: any): any {`
      );

      let isFirst = true;
      for (const [eventName, annotations] of Object.entries(
        perEventAnnotations
      )) {
